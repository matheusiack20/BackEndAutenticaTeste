...
const fs = require('fs');
import { fetchDataWithRetry } from '../../getTopSellers/services/GetTopSellersInfoService';
...
class GenerativeContentService {
  ...
  private async fetchProductDataTopSeller(query: string, type: string) {
    try {
      const { titles, keywords } = await fetchDataWithRetry(query);
      switch (type) {
        case 'T':
          return titles;
        case 'K':
...
// Garantir que sempre seja um JSON válido
  ...
  async generateTitleIdentifier(
    files: 
      ...
      \`\`\``
      return JSON.stringify(await this.generateContent(promptIdentifier,false,files))
  }

  async deleteTemporaryFiles(files: TemporaryFiles | Express.Multer.File[]) {
    try {
      const fileArray = Array.isArray(files)
        ? files
        : Object.values(files).flat();

      await Promise.all(fileArray.map((file) => fs.promises.unlink(file.path)));
    } catch (error) {}
  }

  async generateMetadataTags(
    title: string,
    files:
      | {
          [fieldname: string]: Express.Multer.File[];
  ...
  async generateTitle(
    ...
    const topSellerTitle = await this.fetchProductDataTopSeller(title, 'T');
...
export { GenerativeContentService };
